...
`organization_id` on every tenant table; repositories take an organization scope and refuse to run without one. Cross-tenant row reads and writes are forbidden. ADR-011 defines one fixed platform-wide aggregate report with no tenant identifiers or row access; it is not a general admin bypass. Row-level security policies and a `postbag_app` role ship in the migrations as a second fence. Public submit runs on a narrow, audited path. Plan limits are checked at creation (forms, destinations) and counted per month (submissions) with soft-fail: over-limit Submissions are stored and flagged. After capacity returns, a manager explicitly releases each one to queue Delivery.
