import { signWebhook } from "@postbag/core"
import { z } from "zod"
...
export function createWebhookAdapter(): DestinationAdapter<WebhookConfig> {
  const deliver: DestinationAdapter<WebhookConfig>["deliver"] = async (config, payload, ctx) => {
    ...
    const headers: Record<string, string> = {
      ...
      "Postbag-Delivery": ctx.deliveryId,
      "Postbag-Event": ctx.eventType,
      ...config.headers,
    }
