...
integration("/v1 API", () => {
  ...
  const signedDownloadRequests: string[] = []
  const storage: ObjectStorage = {
    put: () => Promise.resolve(),
    delete: () => Promise.resolve(),
    signedDownloadUrl(key) {
      signedDownloadRequests.push(key)
  ...
  it("returns tenant-scoped attachment metadata and an authorized private download", async () => {
    ...
    const detail = await harness.app.request(`/v1/submissions/${submissionId}`, authed(keyA))
      ...
      attachments: [
        {
          id: attachmentId,
          filename: "screen.png",
          download_url: `/v1/attachments/${attachmentId}/download`,
        },
      ],
    })
    ...
    expect(download.headers.get("location")).toBe(
      `https://storage.example/${storageKey}?signed=true`,
    )
    expect(signedDownloadRequests).toContain(storageKey)

    const noStorageHarness = buildHarness({}, {}, undefined, null)
    try {
      const unavailable = await noStorageHarness.app.request(
        `/v1/attachments/${attachmentId}/download`,
        authed(readOnlyKey),
      )
  ...
  it("reports only non-test submissions from the current month as monthly usage", async () => {
    ...
    try {
      ...
      expect(response.status).toBe(200)
      const body = (await response.json()) as {
        limits: { used: { submissions_this_month: number } }
      }
  ...
  it("enforces configured form and destination limits", async () => {
    ...
    try {
      ...
      expect(((await secondForm.json()) as { error: { code: string } }).error.code).toBe(
        "plan_limit_reached",
      )
      ...
      expect(((await secondDestination.json()) as { error: { code: string } }).error.code).toBe(
        "plan_limit_reached",
      )
      ...
      const rows = await db
        .select()
        .from(destinations)
        .where(eq(destinations.organizationId, limitedOrg.organizationId))
    ...
    const response = await harness.app.request(
      ...
      authed(keyA, {
        ...
        body: JSON.stringify({
          ...
          origin: "https://Example.com/contact/",
        }),
      }),
    )
  ...
  it("publishing a form schema bumps the version and resolves matching drift events", async () => {
    const createResponse = await harness.app.request(
      ...
      authed(keyA, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: "Schema publish test" }),
      }),
    )
  ...
  it("derives a stream's first schema from the first attached form's published schema, with an identity mapping", async () => {
    ...
    const changed = await db.select().from(events).where(eq(events.organizationId, orgAId))
    expect(
      changed.some(
        (e) =>
          e.type === "stream.schema.changed" &&
          (e.subject as { stream_id?: string }).stream_id === stream.id,
      ),
    ).toBe(true)
    ...
    const streamResponse = await harness.app.request(
      ...
      authed(keyA, {
        ...
        body: JSON.stringify({
          ...
          sources: [
            { selector: "tag:selector-preview", mapping: { email: { from: "contact.email" } } },
          ],
        }),
      }),
    )
  ...
  it("accepts only non-empty tag and project selectors for Stream sources", async () => {
    ...
    expect(((await attachProjectSelector.json()) as { selector: string }).selector).toBe(
      "project:prj_site",
    )
    ...
    expect(
      ((await invalidAttach.json()) as { error: { code: string; docs?: string } }).error,
    ).toMatchObject({
  ...
  it("counts recent Submissions from tag and Project selector sources once per Stream", async () => {
    ...
    try {
      ...
      const [taggedForm, projectForm, bothSelectorForm, unrelatedForm] = await db
        ...
        .returning()
      if (
        taggedForm === undefined ||
        projectForm === undefined ||
        bothSelectorForm === undefined ||
        unrelatedForm === undefined
      )
      ...
      expect(detail.status).toBe(200)
      expect(
        ((await detail.json()) as { counts: { submissions_30d: number } }).counts.submissions_30d,
      ).toBe(3)
    } finally {
  ...
  it("derives a stream's first schema from recent submissions when the form has no schema", async () => {
    ...
    expect(Object.keys(detailBody.schema?.json_schema.properties ?? {}).sort()).toEqual([
      "company",
      "fullName",
    ])
    expect(detailBody.sources?.[0]?.mapping_status).toBe("valid")
    expect(detailBody.sources?.[0]?.mapping).toEqual({
  ...
  it("returns 422 stream_schema_missing when neither the stream nor the form has any fields", async () => {
    ...
    expect(attach.status).toBe(422)
    const body = (await attach.json()) as {
      error: { code: string; hint: string; details?: { form_id?: string } }
    }
  ...
  it("names an unnamed destination after where it sends", async () => {
    ...
    const webhook = await harness.app.request(
      ...
      authed(keyA, {
        ...
        body: JSON.stringify({
          type: "webhook",
          config: { url: "https://hooks.example.com/postbag/leads" },
        }),
      }),
    )
  ...
  it("rejects incomplete digest Route updates without changing delivery mode", async () => {
    ...
    expect(((await unchanged.json()) as { mode: { type: string } }).mode).toEqual({
      type: "instant",
    })
  ...
  it("GET /v1/webhooks/{id}/deliveries returns cursor-paginated delivery history", async () => {
    ...
    const body = (await list.json()) as {
      data: { id: string; status: string; webhook_id: string }[]
    }
    ...
    const fromOrgB = await harness.app.request(
    ...
    await db
      .delete(systemWebhookDeliveries)
      .where(eq(systemWebhookDeliveries.webhookId, webhook.id))
  })
