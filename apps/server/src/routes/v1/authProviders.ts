...
const authProvidersRoute = createRoute({
  ...
  description:
    "Public, unauthenticated — no API key needed. Email+password is always available. `social` " +
...
export function registerAuthProviderRoutes(app: OpenAPIHono<AppEnv>, env: Env): void {
  app.openapi(authProvidersRoute, (c) => {
    ...
    if (env.GOOGLE_CLIENT_ID !== undefined && env.GOOGLE_CLIENT_SECRET !== undefined) social.push("google")
    if (env.GITHUB_CLIENT_ID !== undefined && env.GITHUB_CLIENT_SECRET !== undefined) social.push("github")
