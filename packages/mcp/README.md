...
agent can begin with a public sandbox, then do the rest with an API key.
...
Requires Node ≥ 22. No key is required to create, test and read a public sandbox. Tenant tools
require a key; mint one with `postbag login` when the sandbox is ready to claim.
...
If no key exists, call **`public_sandboxes_create`** first, use its stable submit URL, then
verify with **`public_sandboxes_get`** and the returned `Sandbox` authorization value. After
`postbag login`, claim it with **`sandboxes_claim`**; that call needs both Bearer auth and the
`Postbag-Sandbox-Token` argument.
...
- **83 generated tools**, one per API operation — `public_sandboxes_create`,
  `public_sandboxes_get`, `sandboxes_claim`, `me_get`, `forms_list`, `forms_create`,
...
Each generated tool's input schema merges the operation's path, query and header params and JSON
body into one flat object. Body fields keep their API names; a path or query param whose name
collides with a body field is exposed as `path_<name>` / `query_<name>` / `header_<name>` instead (rare in
