...
for the single-container self-host). Postgres is the only stateful dependency.
See [ADR-001](./decisions/ADR-001-stack.md).
...
2. Parse body: `application/x-www-form-urlencoded`, `multipart/form-data`, `application/json`. Strip control fields (`_redirect`, `_gotcha`, `_idempotency`, `_subject`).
...
5. **One transaction:** insert submission; compute applicable routes (direct + via streams, respecting `enabled`, `window`, `quality`); insert one `delivery` row per route (`pending`, or `skipped` with reason); insert `submission.received` event.
...
No third-party network call happens in steps 1–6. Turnstile verification is the one
exception and is bounded by a short timeout with fail-open-to-quarantine.
...
- Housekeeping: schema inference for `observe` forms, retention deletion, destination health (`failing` after N consecutive failures).
